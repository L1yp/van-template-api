...
public class LogAspect {
    ...
    public Object aroundAdvice(JoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (record) {
                model.setToken(StpUtil.stpLogic.getTokenValue());
                model.setToken(StpUtil.getTokenValue());
    ...
    public static UserOperateLogDO getOperateLogModel() {
        UserOperateLogDO model = new UserOperateLogDO();
        RequestAttributes requestAttributes = RequestContextHolder.getRequestAttributes();
        if (requestAttributes == null) {
            return null;
        }
        ...
        if (requestAttributes instanceof ServletRequestAttributes servletRequestAttributes) {
            ...
            if (request instanceof ContentCachingRequestWrapper requestWrapper) {
                try {
                    byte[] requestBody = requestWrapper.getInputStream().readAllBytes();
                    String characterEncoding = request.getCharacterEncoding();
                    if (requestBody.length > 0) {
                        model.setParam(new String(requestBody, characterEncoding));
                    }
